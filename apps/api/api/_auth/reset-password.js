...
export default async function handler(req, res) {
  ...
  if (user.otp_code !== otpCode) {
    await query('UPDATE users SET otp_attempts = otp_attempts + 1 WHERE id = $1::int', [user.id]);
    return res.status(400).json({ error: 'GeÃ§ersiz veya sÃ¼resi dolmuÅŸ kod.' });
  }

  const passwordHash = await bcrypt.hash(newPassword, 12);
  await query(
    'UPDATE users SET password_hash = $1, otp_code = NULL, otp_expiry = NULL, otp_attempts = 0 WHERE id = $2::int',
